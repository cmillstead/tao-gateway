...
- 2026-03-13: Adversarial code review round 5 — 8 issues found and fixed (3 HIGH, 3 MEDIUM, 2 LOW): background sync task leaked on post-start init failure (H1), id()-based cache key unsafe due to GC address reuse (H2), shutdown aclose_session failure skips remaining cleanup (M1), timed-out executor threads exhaust default pool — use dedicated ThreadPoolExecutor (M2), health cache benign race documented (M3), test slow_metagraph sleep reduced from 1s to 0.2s (L1), internal sync error strings sanitized in health API (L2). Also added sync_generation counter and cache invalidation test. 95 tests passing.
